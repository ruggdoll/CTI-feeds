...
| [Unit42 Timely Threat Intel — feed MISP (esoadamo)](https://gitlab.com/esoadamo-org/unit42-timely-threat-intel-misp-feed) | §11 | `https://esoadamo-org.gitlab.io/unit42-timely-threat-intel-misp-feed/static/misp/` | 436 événements, 2020-08-20 → 2026-09-18 | **réemballage** : les rapports texte du dépôt `Unit42-timely-threat-intel` (§7.1) convertis en événements MISP par extraction automatique (modèles de langage) ; l'auteur signale ses limites dans le README |
| [Malicious Extensions Threat Intel Feed (esoadamo)](https://gitlab.com/esoadamo-org/malicious-extensions-threat-intelligence-feed) | §11 | `https://esoadamo-org.gitlab.io/malicious-extensions-threat-intelligence-feed/static/misp/` | 3 événements (Chrome, Firefox, VS Code), 2026-09-19 | extensions malveillantes publiées par le compte Mastodon `@malicious_browser_bot` (infosec.exchange), avec score ; **réemballage** |
...
| [CiviCERT](https://www.civicert.org/apply-to-be-a-member/) | §5 | instance MISP réservée aux membres (organisations d'assistance à la société civile), sur candidature |
...
| [CIRCL — feed OSINT en STIX 2.1](https://codeberg.org/adulau/misp-circl-feed) | §2.1 | `feeds/circl/stix-2.1/<uuid>.json` (Codeberg) | un bundle par événement (ex. 991 objets) ; dernier commit 2026-02-02 | conversion du feed MISP par `misp-stix`, publiée par CIRCL. Une conversion plus ancienne en **STIX 2.0** reste servie sur `circl.lu/doc/misp/feed-osint-stix2/` (847 bundles, dernier 2017-11) |
...
| [Daily Threat Intelligence STIX 2.1 (elsiddik)](https://huggingface.co/datasets/elsiddik/daily-threat-intel-stix21) | — | `data/stix21_bundle_latest.json` (Hugging Face, miroir Kaggle) | 500 `indicator`, 2026-08-22 ; Apache 2.0 | vulnérabilités et indicateurs collectés par des agents automatisés (CVE, EPSS, KEV) ; pas d'objet de contexte |
...
- **Un réemballage n'est pas une source.** Rösti, APTtrail, xfeeds, TI-Collector, ThreatCluster et les feeds esoadamo livrent du STIX ou du MISP fabriqué à partir des rapports ou des listes d'autrui, et le disent (« Repackaged », « republished from upstream feeds », « AI models extract IOCs »). La provenance à retenir est celle de l'amont ; quand l'extraction est faite par un modèle de langage, elle est à recouper.
...
- **CISA — avis conjoints (§B.1) : bundles STIX 2.1 mal formés, testés le 2026-09-26.** AA26-204A (113 objets) porte un `Report.object_refs` terminé par deux `None` littéraux — rejeté par un import strict tant qu'ils ne sont pas retirés à la main — et une relation référençant une identité absente du bundle (`MISSING_REFERENCE_ERROR`). Une fois nettoyé, l'import crée des doublons non réconciliés d'objets déjà connus sous un autre nom ou type ailleurs dans le graphe (`Evilginx`/`Evilginx2` en `malware` alors qu'ils existent déjà en `tool` ; `Laundry Bear` en `threat-actor` autonome alors qu'il est déjà l'alias sourcé d'un `intrusion-set` existant), sans confiance ni auteur renseignés sur ces objets. AA25-141B (556 objets, LummaC2) est pire : la même famille y est typée à la fois `malware` et `threat-actor` sous le nom `LummaC2`, un maliciel connu (Agent Tesla) y est typé `threat-actor`, et une entité `malware` y est nommée littéralement `n/a` — signe d'un export automatisé non relu par l'éditeur avant publication. Utilisable, mais seulement après nettoyage des références nulles/orphelines et une revue manuelle des types avant toute fusion avec un graphe existant.
- **Cisco Talos — IOCs (§B.1) : réexport MISP, les noms d'entités collent l'identifiant MITRE, testé le 2026-09-26.** Sur les 134 bundles, 48 noms distincts de `malware`/`threat-actor`/`intrusion-set`/`tool` — Talos réexporte ses attributs MISP en STIX en accolant l'ID logiciel/groupe au nom (`"Emotet - S0367"`, `"PlugX - S0013"`, `"Cobalt Strike - S0154"`) plutôt que de garder le nom seul, si bien qu'une bonne moitié de ces 48 noms sont soit des collisions directes avec un objet déjà connu sous le même nom (`APT41`, `LockBit` — ce dernier existant même en `malware` ET `intrusion-set` sous ce nom), soit des doublons du même objet sous un nom différent (`"Emotet - S0367"` double `Emotet`, `"QakBot - S0650"` double `QakBot`, etc. — une vingtaine de cas recensés). Le dépôt lui-même est incohérent sur sa propre convention (`"Cobalt Strike"` et `"Cobalt Strike - S0154"` coexistent, de même `"IcedID"`/`"IcedID - S0483"`, `"Silence"`/`"Silence group"`). Utilisable pour sa vraie valeur — indicateurs, rapports, techniques — à condition d'écarter les objets `malware`/`threat-actor`/`intrusion-set`/`tool` et les relations qui les touchent avant fusion avec un graphe existant ; ne jamais les importer tels quels.
